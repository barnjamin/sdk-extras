...
class EvalDelta {
    ...
    get_obj_for_encoding(){
        ...
        if(this.global_delta.length>0) obj["gd"] = this.global_delta.map((gd)=>{ return gd.get_obj_for_encoding() })
        if(Object.keys(this.local_deltas).length>0) obj["ld"] = {}
        if(this.logs.length>0) obj["lg"] = this.logs
        if(this.inner_txns.length>0) obj["itx"] = this.inner_txns.map((itxn)=>{ return itxn.get_obj_for_encoding() })
...
class ApplyData {
    ...
    static fromMsgp(apply_data: any): ApplyData {
        ...
        if("ca" in apply_data){
            ad.closing_amount = apply_data["ca"]
        }
        if("aca" in apply_data){
            ad.asset_closing_amount = apply_data["aca"]
        }
        if("rs" in apply_data){
            ad.sender_rewards = apply_data["rs"]
        }
        if("rr" in apply_data){
            ad.receiver_rewards = apply_data["rr"]
        }
        if("rc" in apply_data){
            ad.close_rewards = apply_data["rc"]
        }
        if("caid" in apply_data){
            ad.config_asset = apply_data["caid"]
        }
        if("apid" in apply_data){
            ad.application_id = apply_data["apid"]
        }
        if("dt" in apply_data){
...
class SignedTransactionWithAD  {
    ...
    constructor(gh: Buffer, gen: string, stib: any) {
        ...
        const stxn = {txn: algosdk.Transaction.from_obj_for_encoding(t)} as algosdk.SignedTransaction
...
(async () => {
  ...
  for(const stxn of block.block.txns){
    const stwad = new SignedTransactionWithAD(block.block.gh, block.block.gen, stxn)
...
async function verifyProofHash(txnroot: string, stxn: SignedTransactionWithAD): Promise<boolean>{
